...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		'author' => '<p class="comment-form-author">' . '<label for="author">' . __( '名前' ) . ( $req ? ' <span style="color:red;">*</span>' : '' ) . '</label><br> ' .
		            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
		'email'  => '<p class="comment-form-email"><label for="email">' . __( 'メールアドレス' ) . ( $req ? ' <span style="color:red;">*</span>' : '' ) . '</label><br> ' .
		...
		'url'    => '<p class="comment-form-url"><label for="url">' . __( 'ウェブサイト' ) . '</label><br> ' .
		            '<input id="url" name="url" ' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>',
	);

	$required_text = sprintf( ' ' . __('%s が付いている欄は必須項目です'), '<span style="color:red;">*</span>' );
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( 'コメント<span style="color:red;">*</span>', 'noun' ) . '</label><br> <textarea id="comment" name="comment" rows="6" style="width: 99%;height: 8em;" aria-describedby="form-allowed-tags" aria-required="true"></textarea></p>',
		/** This filter is documented in wp-includes/link-template.php */
		'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		/** This filter is documented in wp-includes/link-template.php */
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>' ), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="comment-notes"><span id="email-notes">' . __( 'メールアドレスが公開されることはありません。' ) . '</span>'. ( $req ? $required_text : '' ) . '</p>',
			...
			<div id="respond" class="comment-respond">
				...
				<?php else : ?>
					<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>" class="comment-form"<?php echo $html5 ? ' novalidate' : ''; ?>>
						...
						?>
						<p class="submit form-submit">
							<input name="<?php echo esc_attr( $args['name_submit'] ); ?>" src="http://www.landcreation.co.jp/static/img/blog/btn_com_go.png" alt="送信する"type="image" id="<?php echo esc_attr( $args['id_submit'] ); ?>" class="<?php echo esc_attr( $args['class_submit'] ); ?>" value="<?php echo esc_attr( $args['label_submit'] ); ?>" />
