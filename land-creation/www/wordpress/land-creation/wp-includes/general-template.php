...
function get_archives_link($url, $text, $format = 'html', $before = '', $after = '') {
	...
	elseif ('html' == $format)
		$link_html = "\t$before<a href='$url'>$text</a>$after<br>\n";
	else // custom
	...
	if ( 'monthly' == $r['type'] ) {
		...
		if ( $results ) {
			...
			foreach ( (array) $results as $result ) {
				...
				$text = sprintf( __( '%1$d年%2$d月' ), $result->year , $result->month );
...
function get_calendar($initial = true, $echo = true) {
	...
	$calendar_caption = _x('%1$s年%2$d月', 'calendar caption');
	$calendar_output = '<h5>' . sprintf($calendar_caption, date('Y', $unixmonth), $thismonth) . '</h5>
	<div class="itembody clearfix cal">
	...
	$calendar_output .= "\n\t</tr>\n\t</tbody>\n\t</table></div>";
	$calendar_output .= "<div class=\"cal_nav clearfix\">";
	if ( $previous ) {
		$calendar_output .= "\n\t\t".'<div class="prev"><a href="' . get_month_link($previous->year, $previous->month) . '">&laquo; ' . $wp_locale->get_month_abbrev($wp_locale->get_month($previous->month)) . '</a></div>';
	} else {
		$calendar_output .= "\n\t\t".'<div class="prev" class="pad">&nbsp;</div>';
	}

	if ( $next ) {
		$calendar_output .= "\n\t\t".'<div class="next"><a href="' . get_month_link($next->year, $next->month) . '">' . $wp_locale->get_month_abbrev($wp_locale->get_month($next->month)) . ' &raquo;</a></div>';
	} else {
		$calendar_output .= "\n\t\t".'<div class="next" class="pad">&nbsp;</div>';
	}
...
function get_the_modified_date($d = '') {
	if ( '' == $d )
		$the_time = get_post_modified_time('Y年m月d日', null, null, true);
	else
...
function paginate_links( $args = '' ) {
	...
	for ( $n = 1; $n <= $total; $n++ ) :
		if ( $n == $current ) :
			$page_links[] = "<a class='num naw'>" . number_format_i18n( $n ) . "</a>";
			$dots = true;
		else :
			if ( $args['show_all'] || ( $n <= $end_size || ( $current && $n >= $current - $mid_size && $n <= $current + $mid_size ) || $n > $total - $end_size ) ) :
				...
				/** This filter is documented in wp-includes/general-template.php */
				$page_links[] = "<a class='num' href='" . esc_url( apply_filters( 'paginate_links', $link ) ) . "'>" . number_format_i18n( $n ) . "</a>";
				$dots = true;
