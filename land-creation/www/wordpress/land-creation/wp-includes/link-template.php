...
function get_the_post_navigation( $args = array() ) {
	...
	$navigation = '';
	$blog_list  = '<a class="blgtop" href="/"><img src="http://www.landcreation.co.jp/static/img/blog/btn_blgtop.png" width="103" height="24" alt=""></a>';
	$previous   = get_previous_post_link( '%link', $args['prev_text'] );
	$next       = get_next_post_link( '%link', $args['next_text'] );

	// Only add markup if there's somewhere to navigate to.
	if ( $previous || $next ) {
		$navigation = _navigation_markup( $blog_list . $previous . $next, 'post-navigation', $args['screen_reader_text'] );
	}
