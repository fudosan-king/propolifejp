...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			$template_url = get_template_directory_uri(). '/library/multipostthumbnails/js/multi-post-thumbnails-admin.js';
			...
			if (version_compare($wp_version, '3.5', '<')) {
				...
				wp_enqueue_script( "mpt-featured-image-modal", get_template_directory_uri(). '/library/multipostthumbnails/js/media-modal.js', array( 'jquery', 'media-models' ) );
			}

			wp_enqueue_style( "mpt-admin-css", get_template_directory_uri(). '/library/multipostthumbnails/css/multi-post-thumbnails-admin.css');
		}
	...
	if ( is_admin() )
		load_plugin_textdomain( 'multiple-post-thumbnails', FALSE, get_bloginfo('template_url').'/languages/' );
}
